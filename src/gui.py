...
from src.graph_export import export_graph_to_gexf
...
class App(ctk.CTk):
    ...
    def export_gexf(self):
        ...
        file_path = filedialog.asksaveasfilename(
            title="Save GEXF Graph",
            defaultextension=".gexf",
            filetypes=[("GEXF files", "*.gexf")],
            initialfile="logseq_graph.gexf"
        )
        ...
        def task():
            try:
                (
                    ...
                    total_links,
                    asset_links
                ) = export_graph_to_gexf(file_path)
                ...
                self.output.after(
                    ...
                    lambda: self.log(
                        ...
                        f"Nodes: {nodes}\n"
                        f"Edges: {edges}\n"
                        f"Total Link Mentions: {total_links}\n"
                        f"Asset References: {asset_links}\n\n"
                    )
                )
